...
export default function LikeButton({ initialLikes = [], currentUserId, onToggle }: LikeButtonProps) {
  ...
  return (
    <div className="relative inline-block">
      ...
      <button 
        {...longPressProps}
        className={`flex items-center gap-1.5 transition-all font-bold text-sm select-none
          ${isLikedByMe 
            ? 'text-pink-500' 
            : 'text-slate-400 active:scale-95 [@media(hover:hover)]:hover:text-slate-600' 
          }
        `}
        // EXPLANATION:
        // 'active:scale-95' -> provides a tactile "shrink" effect on tap (works on mobile)
        // '[@media(hover:hover)]:hover:text-slate-600' -> ONLY applies the grey hover color if the device has a mouse.
        // This prevents the "sticky" color change on touchscreens.

        title="Long press to see who liked"
      >
        {isLikedByMe ? (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 animate-in zoom-in duration-200 text-pink-500">
