...
export default function PostCard({ post, currentUserId }: { post: any, currentUserId: string }) {
  ...
  // ... (keep existing state: isEditing, editContent, showComments, commentText)
  ...
  // --- HANDLERS (Keep existing delete, save, toggleLike, comment) ---
  async function handleDelete() { if (confirm('Delete?')) await deletePost(post.id) }
  async function handleSave() { const res = await editPost(post.id, editContent); if(res.success) setIsEditing(false); else alert(res.message); }
  ...
  const cardStyle = isUrgent 
    ? "bg-red-500/5 border-red-200/50 backdrop-blur-sm shadow-md" // Urgent Style
    : "bg-white border-slate-100 shadow-sm" // Regular Style
  ...
  return (
    <div className={`p-6 rounded-xl border mb-6 transition-all hover:border-slate-300 ${cardStyle}`}>
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        {/* Buttons (Edit/Delete) - Only show if Author */}
        {isAuthor && (
