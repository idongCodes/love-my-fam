...
import { deletePost, editPost, toggleLike } from '@/app/common-room/actions' // Import toggleLike
...
export default function PostCard({ 
  ...
  // Initialize state with real database values
  ...
  // ... (handleDelete and handleSave remain exactly the same)
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 mb-6 transition-all hover:border-slate-200">
      ...
      {/* ... (Header and Content sections remain EXACTLY the same as before) ... */}
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isAdmin && <span className="text-[10px] bg-slate-800 text-brand-yellow px-2 py-0.5 rounded-md font-bold uppercase tracking-wider border border-slate-600 flex items-center gap-1">🛡️ Admin</span>}
            </div>
          </div>
        </div>
        ...
        {isAuthor && (
          <div className="flex gap-3 text-slate-400">
            {canEdit && !isEditing && (
              <button onClick={() => setIsEditing(true)} className="hover:text-brand-sky transition-colors p-1 rounded-full hover:bg-slate-50">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
              </button>
            )}
            <button onClick={handleDelete} className="hover:text-red-500 transition-colors p-1 rounded-full hover:bg-red-50">
      ...
      {isEditing ? (
        <div className="mt-2">
          <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-brand-sky outline-none text-slate-700" rows={3} />
      ...
      <div className="mt-5 pt-4 border-t border-slate-50 flex justify-between items-center">
        ...
        {/* Left: Like Button (UPDATED to call handleToggleLike) */}
        <button 
        ...
        {/* Right: Reply Button */}
        <button className="flex items-center gap-2 text-slate-400 hover:text-brand-sky transition-colors font-medium text-sm group">
          <span>Reply</span>
