...
export default function BottomNav() {
  ...
  }, [isLoggedIn, pathname]) // Re-fetch on navigation too
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50">
      ...
      <div className="relative bg-brand-sky/90 backdrop-blur-xl rounded-full shadow-2xl border border-white/30">
        ...
        {/* Navigation content */}
        <div className="relative flex items-center justify-center px-3 md:px-6 py-2 gap-1 md:gap-2">
          {navItems.map((item) => {
            if (item.isButton) {
              return (
                <button
                  ...
                  className={`
                    flex flex-col items-center gap-1 px-2 md:px-4 py-2 rounded-full transition-all duration-300
                    ${isActive(item.href) 
          ...
          {isLoggedIn && (
            <form action={logout} className="flex flex-col items-center gap-1 px-2 md:px-4 py-2">
              <button
                type="submit"
          ...
          {!isLoggedIn && (
            <Link
              href="/login"
              className="flex flex-col items-center gap-1 px-2 md:px-4 py-2 rounded-full transition-all duration-300 text-white/90 hover:text-white hover:bg-white/20 hover:scale-105"
            >
              <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
